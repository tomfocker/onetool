...
type RecorderBridgeResponse<T> = { success: boolean; data?: T }
...
type ScreenRecorderBootstrapLoaders = {
  getDefaultPath?: (format?: RecorderFormat) => Promise<RecorderBridgeResponse<string>>
  getHotkey?: () => Promise<RecorderBridgeResponse<string>>
  getSession?: () => Promise<RecorderBridgeResponse<RecorderSessionUpdate>>
  loadScreens: () => Promise<void>
  scheduleScreenLoad?: (callback: () => void) => void
}

type ScreenRecorderBootstrapCallbacks = {
  setOutputPath: (value: string) => void
  setRecorderHotkey: (value: string) => void
  applyAuthoritativeSession: (session: RecorderSessionUpdate) => void
}

function scheduleRecorderScreenHydration(callback: () => void) {
...
export async function bootstrapScreenRecorder(
  loaders: ScreenRecorderBootstrapLoaders,
  callbacks: ScreenRecorderBootstrapCallbacks
) {
  const [defaultPathResult, hotkeyResult, sessionResult] = await Promise.allSettled([
    loaders.getDefaultPath ? loaders.getDefaultPath() : Promise.resolve({ success: false } as RecorderBridgeResponse<string>),
    loaders.getHotkey ? loaders.getHotkey() : Promise.resolve({ success: false } as RecorderBridgeResponse<string>),
    loaders.getSession ? loaders.getSession() : Promise.resolve({ success: false } as RecorderBridgeResponse<RecorderSessionUpdate>)
  ])

  if (defaultPathResult.status === 'fulfilled' && defaultPathResult.value.success && defaultPathResult.value.data) {
    callbacks.setOutputPath(defaultPathResult.value.data)
  }

  if (hotkeyResult.status === 'fulfilled' && hotkeyResult.value.success && hotkeyResult.value.data) {
    callbacks.setRecorderHotkey(hotkeyResult.value.data)
  }

  if (sessionResult.status === 'fulfilled' && sessionResult.value.success && sessionResult.value.data) {
    callbacks.applyAuthoritativeSession(sessionResult.value.data)
  }

  const scheduleScreenLoad = loaders.scheduleScreenLoad ?? scheduleRecorderScreenHydration
  scheduleScreenLoad(() => {
    void loaders.loadScreens()
  })
}
...
export function useScreenRecorder() {
  ...
  const [completedTasks, setCompletedTasks] = useState<CompletedRecordingTask[]>([])
  ...
  const lastAutoGeneratedPathRef = useRef<string | null>(null)
  ...
  const loadCompletedTasks = useCallback(async () => {
    if (!window.electron?.screenRecorder?.getCompletedTasks) {
      return []
    }

    setCompletedTasksLoading(true)
    try {
      const response = await window.electron.screenRecorder.getCompletedTasks()
      if (response.success && response.data) {
  ...
  const openCompletedTask = useCallback(async (
    id: string,
    action: CompletedRecordingOpenAction
  ) => {
    if (!window.electron?.screenRecorder?.openCompletedTask) {
      return { success: false, error: '当前环境不支持打开录制任务' }
    }

    return window.electron.screenRecorder.openCompletedTask(id, action)
  }, [])
  ...
  const handleModeChange = useCallback(async (mode: RecorderMode) => {
    ...
    if (mode === 'full') {
      if (window.electron?.screenRecorder?.hideSelectionPreview) {
        await window.electron.screenRecorder.hideSelectionPreview()
      }
      await loadScreens()
      return
    }
  ...
  useEffect(() => {
    const init = async () => {
      ...
      await bootstrapScreenRecorder(
        {
          getDefaultPath: (nextFormat) => window.electron.screenRecorder.getDefaultPath(nextFormat ?? 'mp4'),
          getHotkey: window.electron.screenRecorder.getHotkey,
          getSession: window.electron.screenRecorder.getSession,
          loadScreens
        },
        {
          setOutputPath,
    ...
    void init()
  }, [applyAuthoritativeSession, loadCompletedTasks, loadScreens])

  useEffect(() => {
    if (!outputPath || !AUTO_GENERATED_RECORDER_PATH_PATTERN.test(outputPath)) {
      return
    }

    if (lastAutoGeneratedPathRef.current && outputPath !== lastAutoGeneratedPathRef.current) {
      return
    }

    const nextOutputPath = ensureRecorderOutputPath(outputPath, format)
    lastAutoGeneratedPathRef.current = nextOutputPath
