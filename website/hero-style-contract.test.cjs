...
test('hero title uses launch-page typography instead of the old stacked tower', () => {
  ...
  assert.match(style, /max-width:\s*9\.4ch/)
  assert.match(style, /font-family:\s*'Noto Sans SC', 'Manrope', 'PingFang SC', sans-serif/)
  assert.match(style, /font-size:\s*clamp\(3\.25rem,\s*5\.9vw,\s*6rem\)/)
  assert.match(style, /font-weight:\s*800/)
  assert.match(style, /letter-spacing:\s*-0\.082em/)
  assert.match(style, /\.hero-title-line-wide\s*\{[\s\S]*font-size:\s*0\.84em/)
})
