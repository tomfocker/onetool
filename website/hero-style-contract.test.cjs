...
test('hero flight uses the new eight-card ring and grouped receiver offsets', () => {
  ...
  assert.match(style, /contain:\s*layout paint/)
...
test('hero title uses launch-page typography instead of the old stacked tower', () => {
  ...
  assert.match(style, /max-width:\s*9ch/)
  assert.match(style, /font-family:\s*'Noto Sans SC', 'Manrope', 'PingFang SC', sans-serif/)
  assert.match(style, /font-size:\s*clamp\(3\.18rem,\s*5\.8vw,\s*5\.86rem\)/)
  assert.match(style, /font-weight:\s*780/)
  assert.match(style, /letter-spacing:\s*-0\.092em/)
  assert.match(style, /\.hero-title-line-wide\s*\{[\s\S]*font-size:\s*0\.82em/)
})
