...
test('hero cards map to tool groups and scroll timing comes only from toolsSection', () => {
  ...
  const dockOffsetBlock = script.match(/const getDockVisualOffset = \(targetKey, state\) => \{[\s\S]*?\n  \}/)
  const applyFlightGeometryBlock = script.match(/const applyFlightGeometry = \(state\) => \{[\s\S]*?\n  \}/)
  ...
  assert.ok(dockOffsetBlock, 'expected getDockVisualOffset block in script.js')
  ...
  assert.match(dockOffsetBlock[0], /const highlight = state\?\.highlight\?\.\[targetKey\] \?\? 0/)
  assert.match(dockOffsetBlock[0], /y:\s*-22 \* highlight/)
  ...
  assert.match(applyFlightGeometryBlock[0], /const dockOffset = getDockVisualOffset\(targetKey, state\)/)
  ...
  assert.match(syncScrollStateBlock[0], /const state = getHeroMotionState\(/)
  assert.match(syncScrollStateBlock[0], /applyFlightGeometry\(state\)/)
